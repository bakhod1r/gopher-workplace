...
func TestConsumeStoresTheValue(t *testing.T) {
	Sink = 0
	if got := Consume(7); got != 0 {
		t.Errorf("Consume(7) = %d, want the previous Sink 0", got)
	}
	if Sink != 7 {
		t.Errorf("Sink = %d, want 7 — the result must be observable outside the package", Sink)
	}
}

func TestConsumeReturnsThePrevious(t *testing.T) {
	Sink = 0
	Consume(7)
	if got := Consume(9); got != 7 {
		t.Errorf("Consume(9) = %d, want 7", got)
	}
	if Sink != 9 {
...
func TestSinkSeesTheBenchmarkResult(t *testing.T) {
	Sink = 0
	for i := 0; i < 5; i++ {
		Consume(SumTo(i))
	}
