...
func TestGuardUsesReadWriteLocks(t *testing.T) {
	targets := map[string]bool{"Get": true, "Set": true}

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "concurmap.go", nil, 0)
	if err != nil {
	...
	if !seen["RLock"] || !seen["Lock"] {
		t.Logf("WARN: Get must use RLock and Set must use Lock")
	}
}
