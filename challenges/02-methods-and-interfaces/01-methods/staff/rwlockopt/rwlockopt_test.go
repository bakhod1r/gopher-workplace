...
func TestGuardEscalatesLock(t *testing.T) {
	targets := map[string]bool{"IncrementIfZero": true}

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "rwlockopt.go", nil, 0)
	if err != nil {
	...
	if !seen["RLock"] || !seen["Lock"] {
		t.Logf("WARN: check under RLock, then escalate to Lock and re-check")
	}
}
