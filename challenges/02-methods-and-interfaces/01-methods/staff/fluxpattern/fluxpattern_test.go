...
func TestGuardDispatchesOnAction(t *testing.T) {
	targets := map[string]bool{"Dispatch": true}

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "fluxpattern.go", nil, 0)
	if err != nil {
	...
	if !seen["action"] || !seen["Count"] {
		t.Logf("WARN: branch on the action parameter and change s.Count accordingly")
	}
}
