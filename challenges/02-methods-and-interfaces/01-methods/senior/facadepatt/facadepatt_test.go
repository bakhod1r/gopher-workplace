...
func TestGuardCallsBothSubsystems(t *testing.T) {
	targets := map[string]bool{"Operation": true}

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "facadepatt.go", nil, 0)
	if err != nil {
	...
	if !seen["Op1"] || !seen["Op2"] {
		t.Logf("WARN: the facade must call both subsystems (Op1 and Op2), not return their results hand-typed")
	}
}
