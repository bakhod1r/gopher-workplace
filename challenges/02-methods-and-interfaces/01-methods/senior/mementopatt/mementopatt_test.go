...
func TestGuardUsesSnapshotState(t *testing.T) {
	targets := map[string]bool{"Save": true, "Restore": true}

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "mementopatt.go", nil, 0)
	if err != nil {
	...
	if !seen["state"] || !seen["Text"] {
		t.Logf("WARN: Save and Restore must move the value through the Memento's state field")
	}
}
