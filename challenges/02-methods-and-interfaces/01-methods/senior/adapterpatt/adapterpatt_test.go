...
func TestGuardUsesLegacyAndAtoi(t *testing.T) {
	targets := map[string]bool{"GetIntData": true}

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "adapterpatt.go", nil, 0)
	if err != nil {
	...
	if !seen["GetStringData"] || !seen["Atoi"] {
		t.Logf("WARN: adapt the legacy value - call GetStringData and parse it with strconv.Atoi, don't return a hand-typed number")
	}
}
