...
Contention is about how many writers want the same lock, not how many writers there are. Hashing the key spreads them over independent locks, and correctness survives because each key always lands on the same one.

## Approach

1. `c.shardFor(key)` to route.
2. Lock that shard, add to its map entry, unlock.
...
With 16 shards and 16 workers, writers collide about a sixteenth as often as with a single lock. `Total` then walks the shards once and sums the per-shard counts for each key.
